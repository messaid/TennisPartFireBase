...
export class RegisterComponent implements OnInit {
  ...
  public onChangesValuesPhone(): void {
  ...
  //   public onChangesValues(): void {
  //     for (const field of CostStatusTableComponent.listFieldsValues()) {
  //         this.CostStatusForm.get(field).valueChanges.subscribe(
  //             val => {
  //                 if (isNullOrUndefined(val) || val === '') {
  //                     this.CostStatusForm.get(field).setErrors({ required: true });
  //                 } else if (isNaN(val) || val < 0) {
  //                     this.CostStatusForm.get(field).setErrors({ invalidNumber: true });
  //                 } else if (!isNaN(val) && parseFloat(val) >= 0) {
  //                     this.CostStatusForm.controls[field]
  //                         .setValue(parseFloat(val).toFixed(4), { emitEvent: false });
  //                 }
  //             }
  //         );
  //     }
